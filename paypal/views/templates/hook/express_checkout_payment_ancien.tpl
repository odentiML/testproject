...
<p class="payment_module">
	<a href="javascript:void(0)" onclick="$('#paypal_payment_form').submit();" id="paypal_process_payment" title="{l s='Pay with PayPal' mod='paypal'}">
		...
		{else}
			{if isset($logos.LocalPayPalHorizontalSolutionPP) && $PayPal_payment_method == $PayPal_integral}
				<img src="{$base_dir_ssl}modules/paypal/paypal.jpg" alt="{$PayPal_content.payment_choice|escape:'htmlall':'UTF-8'}" height="48px" />
			{else}
				<img src="{$base_dir_ssl}modules/paypal/paypal.jpg}" alt="{$PayPal_content.payment_choice|escape:'htmlall':'UTF-8'}" />
			{/if}
			Payer par carte bleue sécurisée
		{/if}
